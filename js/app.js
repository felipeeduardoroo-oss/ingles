...
// ============================================================
const DEFAULT_STATE = () => ({
    currentLevel: 0,          // 0 a 9
    // progress: { "0": { wordIndex: { counter: 0, nextReview: 0, level: 0 } } }
    progress: {},
...
function loadState() {
    try {
        ...
        // Migração: garante que progress existe
...
function getInterval(counter) {
    if (counter <= 0) return 60000;          // 1 min
    if (counter === 1) return 600000;        // 10 min
    if (counter === 2) return 3600000;       // 1 hora
    if (counter === 3) return 86400000;      // 1 dia
    if (counter >= 4) return 259200000;      // 3 dias (mantém)
    return 60000;
}
...
function getAllowedLevels(state) {
    const max = state.currentLevel;
    // Verifica se o atual já está com 80% para liberar o próximo
    let highestUnlocked = 0;
    for (let i = 0; i < TOTAL_LEVELS; i++) {
        if (i === 0) { highestUnlocked = 0; continue; }
        if (isLevelUnlocked(state, i)) highestUnlocked = i;
        else break;
    }
    // Se o usuário já passou de 80% e o currentLevel está defasado, atualiza automático
    for (let i = 0; i < TOTAL_LEVELS; i++) {
        if (isLevelUnlocked(state, i)) {
            if (i > state.currentLevel) state.currentLevel = i;
        }
    }
...
function selectCardsForSession(state) {
    ...
    // Separa palavras com prioridade (erradas ou vencidas)
    const priority = [];
    const newWords = [];
    const stable = [];
    ...
    // Monta 12 cards: 60% prioridade (arredondado) + 40% novos/estáveis
    ...
    // Se ainda faltar, puxa de onde tiver (prioridade ou novos)
    ...
    // Embaralha a sessão para não ficar previsível
    shuffle(selected);
    return selected;
}
...
function renderDashboard() {
    ...
    if (nextLevel < TOTAL_LEVELS) {
        if (isUnlocked) {
            ...
            // Atualiza o currentLevel se necessário
    ...
    // Taxa da sessão atual (se houver)
...
function renderCard() {
    ...
    // Pega distratores: pega palavras de qualquer nível, mas evita duplicatas
    const pool = WORDS.filter(w => w.pt !== correctPt && w.en !== card.en);
    shuffle(pool);
    for (let i = 0; i < 3 && i < pool.length; i++) {
        options.push(pool[i].pt);
    }
    // Se não tiver 4, completa com genéricos
